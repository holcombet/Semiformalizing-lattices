...
After informalize or formalize, before the user accepts a section; when the user asks “is this the same as Lean?”; when deciding whether to edit human `*.md` or only `-ai.md`.
...
Compare: read a semiformal proof (human `*.md` and/or `-ai.md`) and the matching Lean declaration; report alignment by tier; list gaps, extras, and statement mismatches; recommend informalize, formalize, or no change—without editing proofs unless the user explicitly asks.
...
- Misaligned — `S` or `St` fails; needs informalize and/or human remark / statement fix.
...
6. Recommendation — informalize | formalize | none | user decision
