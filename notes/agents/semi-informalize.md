# Agent card: semi-informalize

## Preamble: Lean → semiformal

This repository runs a loop between `.lean` (checked mathematics) and `-ai.md` (assistant semiformal). Semi-informalize is the step from Lean toward semiformal: explain what the formal proof does in the project’s `>` step style (`Ass`, `Have`, `Show`, `Case`, `QED`).
...
For informal prose in human `.md`, use **informalize** (`notes/agents/informalize.md`). For human → `-ai.md` without Lean, use **semiformalize**.
...
### When to semi-informalize

Lean exists but `-ai.md` is missing or stale; after formalize changes a proof; user asks to semi-informalize a declaration (e.g. `lemma_5_3_i`). Suggest `compare` afterward if the user asks whether the result is “close enough”.
...
Semi-informalize: write or refresh semiformal proofs in `SemiformalProof/<Chapter>-ai.md` from `FormalProofs/<Chapter>.lean`. Update the bridge table when done.
...
```
Role: semi-informalize (see notes/agents/semi-informalize.md).
Chapter: 5_FiniteRepresentation.
Task: Semi-informalize lemma_5_3_i into SemiformalProof/5_FiniteRepresentation-ai.md.
