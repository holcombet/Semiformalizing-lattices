...
def ClosedUnderNonemptyInter (L : Set (Set X)) : Prop :=
  ∀ S : Set (Set X), S ⊆ L → S.Nonempty → sInf S ∈ L


def IsInterStructure {X : Type*} (L : Set (Set X)) : Prop :=
  L.Nonempty ∧ ClosedUnderNonemptyInter L

def IsToppedInterStructure {X : Type*} (L : Set (Set X)) : Prop :=
  Set.univ ∈ L ∧ IsInterStructure L

lemma isInterStructure_iff (L : Set (Set X)) :
  IsInterStructure L ↔ L.Nonempty ∧ ClosedUnderNonemptyInter L := by
  constructor
  · intro h
    exact ⟨h.1, h.2⟩
  · intro h
...
lemma isToppedInterStructure_iff (L : Set (Set X)) :
  IsToppedInterStructure L ↔ L.Nonempty ∧ ClosedUnderNonemptyInter L ∧ Set.univ ∈ L := by
  constructor
  · intro h
...
namespace ChainConditionAbbreviations

abbrev ACC (α : Type*) [LT α] : Prop := WellFoundedGT α
abbrev DCC (α : Type*) [LT α] : Prop := WellFoundedLT α

end ChainConditionAbbreviations

open ChainConditionAbbreviations
